...
.directive('awSelect', ['$window', '$compile', function($window, $compile) {
	return {
		restrict: 'E',
		transclude: true,
		templateUrl: base_url+'app/aw-select.tmpl.html',
		scope: {model: '=?ngModel', ngChange: '&', buttonLabels: '=?'},
		controller: function($scope) {
			...
			self.selectOption = function(optionElement) {
				if (self.isMultiple) {
					optionElement.toggleClass('selected');
				} else {
					self.deselectAll();
		...
		controller: function($scope) {
			...
			this.selectGroup = function(optionElement) {
				if (this.isSelectable) {
					...
					angular.forEach(this.optGroupElements, function(elem) {
						allActive ? elem.addClass('selected') : elem.removeClass('selected');
					});
					this.selectCtrl.syncButton(true);
				}
			};
